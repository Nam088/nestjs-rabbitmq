...
describe('RabbitMQService', () => {
      ...
      providers: [
        {
          ...
          useFactory: () => {
            return new RabbitMQService(
              mockConnectionManager as AmqpConnectionManager,
              'test',
            );
          },
        },
  ...
  describe('error handling', () => {
    it('should handle publish errors', async () => {
      ...
      await expect(
        service.publish('test-exchange', 'test.key', { data: 'test' })
    ...
    it('should handle sendToQueue errors', async () => {
      ...
      await expect(
        service.sendToQueue('test-queue', { data: 'test' })
  ...
  describe('request (RPC)', () => {
    ...
    it('should make RPC request and receive response', async () => {
      ...
      if (replyHandler) {
        ...
        const sendCall = mockChannel.sendToQueue.mock.calls.find((call: any) => 
          call[0] === 'rpc-queue'
        );
    ...
    it('should handle RPC request errors', async () => {
      ...
      await expect(
        service.request('rpc-queue', { action: 'test' })
      ).rejects.toThrow('Send failed');
    });
